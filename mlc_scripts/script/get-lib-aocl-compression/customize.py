...
def postprocess(i):
    ...
    if is_true(env.get('MLC_AOCL_LIB_PATH_PROVIDED', '')):
        ...
        if not lib_path or not os.path.isdir(lib_path):
            return {
                'return': 1, 'error': 'Provided MLC_AOCL_COMPRESSION_LIB_PATH does not exist: ' + str(lib_path)}
        ...
        if not env.get('MLC_AOCL_COMPRESSION_VERSION'):
            env['MLC_AOCL_COMPRESSION_VERSION'] = _detect_version_from_path(
                lib_path) or 'unknown'
        return {'return': 0}
    ...
    if is_true(env.get('MLC_AOCL_BINARY_DOWNLOAD', '')):
        ...
        for entry in os.listdir(install_path):
            ...
            if os.path.isdir(entry_path) and (os.path.isdir(os.path.join(
                ...
                break
        env['MLC_AOCL_COMPRESSION_VERSION'] = env.get(
            'MLC_AOCL_BINARY_VERSION', '')
    else:
        # Case 3: source build
        src_path = env.get(
            'MLC_AOCL_COMPRESSION_SRC_PATH', env.get(
                'MLC_GIT_REPO_CHECKOUT_PATH', ''))
        env['MLC_AOCL_COMPRESSION_SRC_PATH'] = src_path
        env['MLC_AOCL_COMPRESSION_BUILD_PATH'] = os.path.join(
            src_path, 'build')
        if env.get('MLC_OUTDIRNAME', ''):
            version = env.get(
                'MLC_AOCL_COMPRESSION_VERSION', env.get(
                    'MLC_GIT_CHECKOUT', 'unknown'))
            install_path = os.path.join(
                env['MLC_OUTDIRNAME'], 'aocl-compression', version)
        else:
