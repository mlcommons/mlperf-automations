...
#define CHECK_CUDA_SUCCESS(x)                                                  \
  if ((x) != cudaSuccess)                                                      \
    std::cerr << "encountered CUDA error" << std::endl;

class GPUDevice : public MlcDevice {
  size_t NumConcurrency() const override { return NumMemory(); }
  ...
  void Read(size_t memory_index, std::vector<uint8_t> &to, const void *from,
    ...
    CHECK_CUDA_SUCCESS(cudaSetDevice(memory_index));
    CHECK_CUDA_SUCCESS(
        cudaMemcpy(to.data(), from, size, cudaMemcpyDeviceToHost));
  }
  void Write(size_t memory_index, void *to,
             const std::vector<uint8_t> &from) override {
    CHECK_CUDA_SUCCESS(cudaSetDevice(memory_index));
    CHECK_CUDA_SUCCESS(
        cudaMemcpy(to, from.data(), from.size(), cudaMemcpyHostToDevice));
  }
