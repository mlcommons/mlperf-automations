...
class Model {
public:
  Model(std::string model_path, size_t num_inputs,
        std::vector<std::string> input_names, std::vector<size_t> input_sizes,
        std::vector<std::vector<size_t>> input_shapes, size_t num_outputs,
        std::vector<std::string> output_names, std::vector<size_t> output_sizes,
        std::vector<std::vector<size_t>> output_shapes)
      : model_path(model_path), num_inputs(num_inputs),
        input_names(input_names), input_sizes(input_sizes),
        input_shapes(input_shapes), num_outputs(num_outputs),
        output_names(output_names), output_sizes(output_sizes),
   ...
   */
  virtual void PostProcess(mlperf::QuerySampleIndex index,
...
public:
  Resnet50(std::string model_path, int64_t argmax_shift)
      : Model(model_path, 1, {"input_tensor:0"},
              {3 * 224 * 224 * sizeof(float)}, {{3, 224, 224}}, 1, {"ArgMax:0"},
              {sizeof(int64_t)}, {{1}}),
        argmax_shift(argmax_shift) {}

  void PostProcess(mlperf::QuerySampleIndex index,
                   const std::vector<void *> &raw,
                   const std::vector<std::vector<size_t>> &raw_shapes,
...
public:
  Retinanet(std::string model_path, size_t width, size_t height,
            float score_threshold)
      : Model(model_path, 1, {"images"}, {3 * width * height * sizeof(float)},
              {{3, width, height}},
              // no fixed output sizes/shapes
              3, {"boxes", "scores", "labels"}, {0, 0, 0}, {{0, 4}, {0}, {0}}),
        width(width), height(height), score_threshold(score_threshold) {}
...
public:
  BertLarge(std::string model_path, size_t max_seq_length)
      : Model(model_path, 3, {"input_ids", "input_mask", "segment_ids"},
              {max_seq_length * sizeof(int64_t),
               max_seq_length * sizeof(int64_t),
               max_seq_length * sizeof(int64_t)},
              {{max_seq_length}, {max_seq_length}, {max_seq_length}}, 2,
