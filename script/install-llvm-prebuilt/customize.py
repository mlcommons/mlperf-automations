...
def preprocess(i):
    ...
    if package_name == '':
        ...
        # If package_name if not forced, attempt to synthesize it based on OS
        # and arch
        if os_info['platform'] == 'darwin':
            ...
            package_name = 'clang+llvm-' + need_version + '-' + force_arch + \
                '-apple-darwin' + force_darwin_version + '.tar.xz'
...
#              else:
                ...
                package_name = 'clang+llvm-' + need_version + \
                    '-x86_64-linux-gnu-ubuntu-' + default_os + '.tar.xz'
