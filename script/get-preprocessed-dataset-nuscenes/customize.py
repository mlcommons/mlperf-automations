...
def postprocess(i):
    ...
    if env.get('MLC_PREPROCESSED_DATASET_NUSCENES_SCENE_LENGTHS_PATH', '') != '':
        shutil.copy(env['MLC_PREPROCESSED_DATASET_NUSCENES_SCENE_LENGTHS_PATH'], os.path.join(env['MLC_PREPROCESSED_DATASET_NUSCENES_PATH'], env['MLC_DATASET_NUSCENES_SCENE_PICKLE_FILENAME']))
    return {'return': 0}
