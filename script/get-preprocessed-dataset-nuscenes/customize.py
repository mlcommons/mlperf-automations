...
def postprocess(i):
    ...
    # Ensure scene_lengths.pkl is alongside the dataset directory regardless
    # of how it was obtained
    dataset_path = env.get(
        'MLC_PREPROCESSED_DATASET_NUSCENES_PATH',
        '').rstrip('/')
    pickle_filename = env.get(
        'MLC_DATASET_NUSCENES_SCENE_PICKLE_FILENAME',
        'scene_lengths.pkl')
    if dataset_path:
        expected = os.path.join(os.path.dirname(dataset_path), pickle_filename)
        if not os.path.exists(expected):
            # Look one level higher (e.g. when downloaded outside the
            # preprocessed dir)
