...
def gui(i):
    ...
    if v is not None and v != '':
        ...
        if v in ['tpu', 'gaudi']:
            st.markdown('----')
            st.markdown(
                '**WARNING: unified CM workflow support for this hardware is pending - please [feel free to help](https://discord.gg/JjWNWXKxwT)!**')
            return {'return': 0, 'skip': True, 'end_html': end_html}

        elif 'orin' in compute_tags:
            st.markdown('----')
            st.markdown(
                '**WARNING: we need to encode CM knowledge from [this Orin setp](https://github.com/mlcommons/ck/blob/master/docs/mlperf/setup/setup-nvidia-jetson-orin.md) to this GUI!**')
            return {'return': 0, 'skip': True, 'end_html': end_html}

    st.markdown('---')
    st.markdown('**How would you like to run the MLPerf inference benchmark?**')

    r = misc.make_selector({'st': st,
                            'st_inputs': st_inputs_custom,
                            'params': params,
                            'key': 'mlperf_inference_device',
                            'desc': inp['device']})
    device = r.get('value2')
    inp['device']['force'] = device

    if device == 'cpu':
        inp['implementation']['choices'] = ['mlcommons-python',
                                            'mlcommons-cpp', 'intel', 'ctuning-cpp-tflite']
        if 'intel' in compute_tags:
            inp['implementation']['default'] = 'intel'
        else:
            inp['implementation']['default'] = 'mlcommons-python'
            inp['backend']['choices'] = [
                'onnxruntime', 'deepsparse', 'pytorch', 'tf', 'tvm-onnx']
        ...
        st.markdown(
            '*WARNING: CM-MLPerf inference workflow was not tested thoroughly for AMD GPU - please feel free to test and improve!*')
    elif device == 'qaic':
    ...
    if division == 'closed':
        ...
        r = misc.make_selector({'st': st,
                                'st_inputs': st_inputs_custom,
                                'params': params,
                                'key': 'mlperf_inference_compliance',
                                'desc': inp[y]})
        ...
        if compliance == 'yes':
            st.markdown(
                '*:red[See [online table with required compliance tests](https://github.com/mlcommons/policies/blob/master/submission_rules.adoc#5132-inference)].*')
    ...
    if v is not None and v != '':
        ...
        if device == 'cuda':
            inp['implementation']['choices'] = [
                'nvidia', 'mlcommons-python', 'mlcommons-cpp']
    ...
    r = misc.make_selector({'st': st,
                            'st_inputs': st_inputs_custom,
                            'params': params,
                            'key': 'mlperf_inference_implementation',
                            'desc': inp['implementation']})
    ...
    if implementation == 'mlcommons-cpp':
        ...
        st.markdown(
            '*:red[[CM automation recipe for this implementation](https://github.com/mlcommons/cm4mlops/tree/main/script/app-mlperf-inference-mlcommons-cpp)]*')
        ...
        if device == 'cuda':
            ...
            inp['backend']['default'] = 'onnxruntime'
        st.markdown(
            '*:red[[CM automation recipe for this implementation](https://github.com/mlcommons/cm4mlops/tree/main/script/app-mlperf-inference-mlcommons-python)]*')
    elif implementation == 'ctuning-cpp-tflite':
        inp['precision']['force'] = 'float32'
        inp['model']['force'] = 'resnet50'
        st.markdown(
            '*:red[[CM automation recipe for this implementation](https://github.com/mlcommons/cm4mlops/tree/main/script/app-mlperf-inference-ctuning-cpp-tflite)]*')
    elif implementation == 'nvidia':
        inp['backend']['force'] = 'tensorrt'
        extra['skip_script_docker_func'] = True
        can_have_docker_flag = True
        st.markdown(
            '*:red[[CM automation recipe for this implementation](https://github.com/mlcommons/cm4mlops/tree/main/script/app-mlperf-inference-nvidia)]*')
...
#        st.markdown('*:red[Note: Intel implementation require extra CM command to build and run Docker container - you will run CM commands to run MLPerf benchmarks there!]*')
        st.markdown(
            '*:red[[CM automation recipe for this implementation](https://github.com/mlcommons/cm4mlops/tree/main/script/reproduce-mlperf-inference-intel)]*')
        ...
        extra['skip_script_docker_func'] = True
        st.markdown(
            '*:red[[CM automation recipe for this implementation](https://github.com/mlcommons/cm4mlops/tree/main/script/reproduce-mlperf-inference-qualcomm)]*')
    ...
    if r['return'] == 0:
        backend_setup = r['string']

    if backend == 'deepsparse':
        inp['model']['choices'] = [
            'resnet50', 'retinanet', 'bert-99', 'bert-99.9']
        inp['model']['default'] = 'bert-99'
        inp['precision']['choices'] = ['float32', 'int8']
        inp['precision']['default'] = 'int8'
        if 'force' in inp['precision']:
            del (inp['precision']['force'])
    ...
    if model == 'retinanet':
        ...
        if implementation == 'mlcommons-python':
            x = '200'
        st.markdown(
            ':red[This model requires ~{}GB of free disk space for preprocessed dataset in a full/submission run!]\n'.format(x))
    ...
    if github_doc_model == '':
        github_doc_model = model

    model_cm_url = 'https://github.com/mlcommons/ck/tree/master/docs/mlperf/inference/{}'.format(
        github_doc_model)
    extra_notes_online = '[Extra notes online]({})\n'.format(model_cm_url)

    st.markdown(
        '*[CM-MLPerf GitHub docs for this model]({})*'.format(model_cm_url))
    ...
    if inp['precision'].get('force', '') == '':
        ...
        r = misc.make_selector({'st': st,
                                'st_inputs': st_inputs_custom,
                                'params': params,
                                'key': 'mlperf_inference_precision',
                                'desc': inp['precision']})
    ...
    choices = [''] + [
        k for k in script_meta_variations if script_meta_variations[k].get(
            'group', '') == 'benchmark-version']
    desc = {
        'choices': choices,
        'default': choices[0],
        'desc': 'Force specific benchmark version?'}
    r = misc.make_selector({'st': st,
                            'st_inputs': st_inputs_custom,
                            'params': params,
                            'key': 'mlperf_inference_version',
                            'desc': desc})
    ...
    if can_have_docker_flag:
        ...
        desc = {
            'choices': choices,
            'default': choices[0],
            'desc': 'Should CM script prepare and run Docker container in interactive mode to run MLPerf? You can then copy/paste CM commands generated by this GUI to benchmark different models.'}
        r = misc.make_selector({'st': st,
                                'st_inputs': st_inputs_custom,
                                'params': params,
                                'key': 'mlperf_inference_docker',
                                'desc': desc})
    ...
    submission = st.toggle(
        'Would you like to prepare official submission?',
        value=False)
    if submission:
        r = misc.make_selector({'st': st,
                                'st_inputs': st_inputs_custom,
                                'params': params,
                                'key': 'mlperf_inference_hw_name',
                                'desc': inp['hw_name']})
        ...
        r = misc.make_selector({'st': st,
                                'st_inputs': st_inputs_custom,
                                'params': params,
                                'key': 'mlperf_inference_submitter',
                                'desc': inp['submitter']})
        ...
        choices = [
            'Performance',
            'Accuracy',
            'Find Performance from a short run',
            'Performance and Accuracy']
        desc = {
            'choices': choices,
            'default': choices[0],
            'desc': 'What to measure?'}
        r = misc.make_selector({'st': st,
                                'st_inputs': st_inputs_custom,
                                'params': params,
                                'key': 'mlperf_inference_measure',
                                'desc': desc})
        ...
        desc = {
            'choices': choices,
            'default': choices[0],
            'desc': 'Which scenario(s)?'}
        r = misc.make_selector({'st': st,
                                'st_inputs': st_inputs_custom,
                                'params': params,
                                'key': 'mlperf_inference_scenario',
                                'desc': desc})
    ...
    desc = {
        'choices': choices,
        'default': choices[0],
        'desc': 'Short (test) or full (valid) run?'}
    r = misc.make_selector({'st': st,
                            'st_inputs': st_inputs_custom,
                            'params': params,
                            'key': 'mlperf_inference_how',
                            'desc': desc})
    how = r.get('value2')
...
#    power = r.get('value2', False)
    ...
    if power:
        ...
        r = misc.make_selector({'st': st,
                                'st_inputs': st_inputs_custom,
                                'params': params,
                                'key': 'mlperf_inference_power_server',
                                'desc': inp[y]})
        ...
        r = misc.make_selector({'st': st,
                                'st_inputs': st_inputs_custom,
                                'params': params,
                                'key': 'mlperf_inference_power_port',
                                'desc': inp[y]})
        inp[y]['force'] = r.get('value2')

        st.markdown(
            '*:red[See [online notes](https://github.com/mlcommons/ck/blob/master/docs/tutorials/mlperf-inference-power-measurement.md)] to setup power meter and server.*')
...
#    dashboard = r.get('value2', False)
    ...
    if dashboard:
        ...
        r = misc.make_selector({'st': st,
                                'st_inputs': st_inputs_custom,
                                'params': params,
                                'key': 'mlperf_inference_power_wb_project',
                                'desc': inp[y]})
        inp[y]['force'] = r.get('value2')
        ...
        r = misc.make_selector({'st': st,
                                'st_inputs': st_inputs_custom,
                                'params': params,
                                'key': 'mlperf_inference_power_wb_user',
                                'desc': inp[y]})
        inp[y]['force'] = r.get('value2')
    ...
    reproduce = st.toggle(
        'Record extra info for reproducibility?',
        value=value_reproduce)

    explore = st.toggle(
        'Explore/tune benchmark (batch size, threads, etc)?',
        value=False)
    ...
    ##########################################################################
    debug = st.toggle(
        'Debug and run MLPerf benchmark natively from command line after CM auto-generates CMD?',
        value=False)
    if debug:
