...
def main():
    ...
    if summary_csv == '':
        raise RuntimeError(
            'MLC_MLPERF_TRAINING_RESULTS_SUMMARY_CSV is not set')
    ...
    try:
        ...
        raise RuntimeError(
            'Python executable not found: {}'.format(
                cmd[0])) from exc
