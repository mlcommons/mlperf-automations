...
def preprocess(i):
    ...
    if env.get('MLC_MMDET_TASK', '') == "automotive":
        ...
        run_cmd = f"{env['MLC_PYTHON_BIN_WITH_PATH']} -m pip install -r requirements/optional.txt && MMCV_WITH_OPS={env['MMCV_WITH_OPS']} pip3 install -e . {env.get('MLC_PYTHON_PIP_COMMON_EXTRA', '')}"
