...
def preprocess(i):
    ...
    os_info = i['os_info']
    env = i['env']
    # automotive loadgen is yet to be uploaded to pypi
    if is_true(env.get('MLC_TMP_MLPERF_AUTOMOTIVE_LOADGEN_INSTALL_FROM_PIP',
               '')):
...
#    if os_info['platform'] != 'windows':
    ...
    env['MLC_MLPERF_AUTOMOTIVE_LOADGEN_INSTALL_PATH'] = install_path

    build_path = os.path.join(cur_path, 'build')
    if os.path.exists(build_path):
        env['MLC_MLPERF_AUTOMOTIVE_LOADGEN_BUILD_PATH'] = build_path
    ...
    env['MLC_MLPERF_AUTOMOTIVE_LOADGEN_INCLUDE_PATH'] = include_path

    env['+LD_LIBRARY_PATH'].append(lib_path)
    env['+DYLD_FALLBACK_LIBRARY_PATH'].append(lib_path)
    env['MLC_MLPERF_AUTOMOTIVE_LOADGEN_LIBRARY_PATH'] = lib_path

    env['+PYTHONPATH'].append(python_path)
    env['MLC_MLPERF_AUTOMOTIVE_LOADGEN_PYTHON_PATH'] = python_path
