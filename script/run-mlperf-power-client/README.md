# README for run-mlperf-power-client
...
```bash
mlcr run,mlc,mlcommons,mlperf,power,client,power-client
```
...
| `--port` |  |  | `` |
| `--power_server` |  |  | `localhost` |
| `--run_cmd` |  |  | `` |
| `--server` | Alias for power_server |  | `` |
| `--server_port` | Alias for port |  | `` |
| `--timestamp` |  |  | `` |
