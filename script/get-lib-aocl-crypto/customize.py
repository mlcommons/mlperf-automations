...
def postprocess(i):
    ...
    if is_true(env.get('MLC_AOCL_LIB_PATH_PROVIDED', '')):
        ...
        if not lib_path or not os.path.isdir(lib_path):
            return {
                'return': 1, 'error': 'Provided MLC_AOCL_CRYPTO_LIB_PATH does not exist: ' + str(lib_path)}
        ...
        if not env.get('MLC_AOCL_CRYPTO_VERSION'):
            env['MLC_AOCL_CRYPTO_VERSION'] = _detect_version_from_path(
                lib_path) or 'unknown'
        return {'return': 0}
    ...
    if is_true(env.get('MLC_AOCL_BINARY_DOWNLOAD', '')):
        ...
        # Case 3: source build
        src_path = env.get(
            'MLC_AOCL_CRYPTO_SRC_PATH', env.get(
        ...
        if env.get('MLC_OUTDIRNAME', ''):
            version = env.get(
                'MLC_AOCL_CRYPTO_VERSION', env.get(
                    'MLC_GIT_CHECKOUT', 'unknown'))
            install_path = os.path.join(
                env['MLC_OUTDIRNAME'], 'aocl-crypto', version)
        else:
            install_path = os.path.join(src_path, 'install')
        if not env.get('MLC_AOCL_CRYPTO_VERSION'):
            env['MLC_AOCL_CRYPTO_VERSION'] = env.get(
