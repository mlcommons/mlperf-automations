...
public:
  TFLiteBenchmark(const BenchmarkSettings *settings,
                  tflite::Interpreter *interpreter, int input_index)
...
public:
  Program() {
    ...
    model =
        tflite::FlatBufferModel::BuildFromFile(settings->graph_file().c_str());
    if (!model)
    ...
    case kTfLiteFloat32:
      if (settings->skip_internal_preprocessing)
        benchmark.reset(new TFLiteBenchmark<float, InCopy, OutCopy>(
            settings, interpreter.get(), input_index));
      else
        benchmark.reset(new TFLiteBenchmark<float, InNormalize, OutCopy>(
            settings, interpreter.get(), input_index));
      break;

    case kTfLiteUInt8:
      benchmark.reset(new TFLiteBenchmark<uint8_t, InCopy, OutDequantize>(
          settings, interpreter.get(), input_index));
      break;
    ...
    cout << format("Input tensor dimensions (NHWC): %d*%d*%d*%d", in_num,
                   in_height, in_width, in_channels)
         << endl;
    if (in_height != settings->image_size || in_width != settings->image_size ||
        in_channels != settings->num_channels)
      throw format("Dimensions of graph's input do not correspond to "
                   "dimensions of input image (%d*%d*%d*%d)",
                   settings->batch_size, settings->image_size,
                   settings->image_size, settings->num_channels);
