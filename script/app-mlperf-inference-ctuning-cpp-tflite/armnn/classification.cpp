...
public:
  ArmNNBenchmark(const BenchmarkSettings *settings, TData *in_ptr,
                 TData *out_ptr)
      : Benchmark<TData, TInConverter, TOutConverter>(settings, in_ptr,
                                                      out_ptr) {}
};
...
public:
  Program() : runtime(armnn::IRuntime::Create(options)) {
    ...
    armnn::INetworkPtr network =
        parser->CreateNetworkFromBinaryFile(settings->graph_file().c_str());
    if (!network)
      throw "Failed to load graph from file";

    armnnTfLiteParser::BindingPointInfo inputBindingInfo =
        parser->GetNetworkInputBindingInfo(0, input_layer_name);
    ...
    armnn::IOptimizedNetworkPtr optNet =
        armnn::Optimize(*network, optOptions, runtime->GetDeviceSpec());
    ...
    if (input_type != output_type)
      throw format(
          "Type of graph's input (%d) does not match type of its output (%d).",
          int(input_type), int(output_type));

    void *input = input_type == armnn::DataType::Float32
                      ? (void *)new float[inSize]
                      : (void *)new uint8_t[inSize];
    void *output = output_type == armnn::DataType::Float32
                       ? (void *)new float[outSize]
                       : (void *)new uint8_t[outSize];
    ...
    case armnn::DataType::Float32:
      if (settings->skip_internal_preprocessing) {
        cout << "************* Type 1" << endl;
        benchmark.reset(new ArmNNBenchmark<float, InCopy, OutCopy>(
            settings, (float *)input, (float *)output));
      } else {
        cout << "************* Type 2" << endl;
        benchmark.reset(new ArmNNBenchmark<float, InNormalize, OutCopy>(
            settings, (float *)input, (float *)output));
      }
      break;

    case armnn::DataType::QAsymmU8:
      benchmark.reset(new ArmNNBenchmark<uint8_t, InCopy, OutDequantize>(
          settings, (uint8_t *)input, (uint8_t *)output));
      break;

    default:
      throw format("Unsupported type of graph's input: %d. "
                   "Supported types are: Float32 (%d), UInt8 (%d)",
                   int(input_type), int(armnn::DataType::Float32),
                   int(armnn::DataType::QAsymmU8));
    }

    int out_num = outShape[0];
    int out_classes = outShape[1];
    cout << format("Output tensor dimensions: %d*%d", out_num, out_classes)
         << endl;
    if (out_classes != settings->num_classes &&
        out_classes != settings->num_classes + 1)
      throw format("Unsupported number of classes in graph's output tensor. "
                   "Supported numbers are %d and %d",
  ...
  // bool is_available_batch() {return session? session->get_next_batch():
  // false; }
  ...
  void ColdRun() {
    ...
    if (runtime->EnqueueWorkload(networkIdentifier, inputTensor,
                                 outputTensor) != armnn::Status::Success)
...
public:
  ...
  void IssueQuery(const std::vector<mlperf::QuerySample> &samples) override {
    ...
    for (auto s : samples) {
      ...
      if (vl > 1) {
        cout << "Query image index: " << s.index
      ...
      */
      ...
      responses.push_back(
          {s.id, uintptr_t(&encoding_buffer[i]), sizeof(encoding_buffer[i])});
      ++i;
    }
  ...
  void ReportLatencyResults(
    ...
    uint64_t avg =
        accumulate(latencies_ns.begin(), latencies_ns.end(), uint64_t(0)) /
        size;
