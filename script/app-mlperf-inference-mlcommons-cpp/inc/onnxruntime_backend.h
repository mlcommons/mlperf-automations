...
public:
  OnnxRuntimeBackend(std::shared_ptr<Model> &model,
    ...
    for (size_t i = 0; i < device->NumMemory(); i++) {
      memory_infos.emplace_back(use_cuda ? "Cuda" : "Cpu",
                                OrtAllocatorType::OrtArenaAllocator, i,
                                OrtMemTypeDefault);
    }
  ...
  void RunInference(size_t concurrency_index,
    ...
    for (size_t i = 0; i < model->num_inputs; i++) {
      ...
      ONNXTensorElementDataType input_type = session.GetInputTypeInfo(i)
                                                 .GetTensorTypeAndShapeInfo()
                                                 .GetElementType();
      Ort::Value value = Ort::Value::CreateTensor(
          memory_infos[memory_index], batch_data[i], size, input_shape.data(),
    ...
    for (size_t i = 0; i < batch.size(); i++) {
      ...
      for (size_t j = 0; j < outputs.size(); j++) {
        ...
        size_t rank =
            outputs[j].GetTensorTypeAndShapeInfo().GetDimensionsCount();
        std::vector<int64_t> output_shape(rank);
        outputs[j].GetTensorTypeAndShapeInfo().GetDimensions(
            output_shape.data(), rank);
...
private:
  ...
  Ort::MemoryInfo memory_info_cpu{Ort::MemoryInfo::CreateCpu(
