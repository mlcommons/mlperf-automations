...
/**
 * A System class represents the policy by which requests from LoadGen are
 * handled.
 *
...
public:
  StreamSUT(std::shared_ptr<MlcBackend> &backend)
      : System("StreamSUT", backend) {}
  ...
  void IssueQuery(const std::vector<mlperf::QuerySample> &samples) override {
    ...
    for (auto batch_begin = samples.begin(); batch_begin < samples.end();
         batch_begin += max_batch_size) {
      auto batch_end = std::min(batch_begin + max_batch_size, samples.end());
      const std::vector<mlperf::QuerySample> batch_queries(batch_begin,
                                                           batch_end);
...
public:
  QueueSUT(std::shared_ptr<MlcBackend> &backend) : System("QueueSUT", backend) {
    // launch threads
