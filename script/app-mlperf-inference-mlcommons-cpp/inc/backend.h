 ...
 * receives queries from SUT and issues them to derived classes via
 * RunInference.
 *
 * For memory storage, on calls to LoadSampleFromRam() from the QSL, loaded
 * samples are stored contiguously into each device memory. The Backend class
 * retains the location of every sample in device memory.
 *
 * When SUT issues a batch to run on a device concurrency, the backend class
 * retrieves the location in memory of this batch, and passes this to
 * RunInference implemented by derived classes (e.g. OnnxRuntimeBackend).
 */
class MlcBackend {
public:
  MlcBackend(std::shared_ptr<Model> &model, std::shared_ptr<MlcDevice> &device,
      ...
      : model(model), device(device),
        ...
        batch_size(batch_size), num_memory(device->NumMemory()),
        num_inputs(model->num_inputs), batch_memory_mutex(num_memory) {
    // have batch_size padding at the end that cycles back to beginning for
    // contiguity
  ...
  size_t NumConcurrency() const { return device->NumConcurrency(); }

  size_t PerformanceSampleCount() const { return performance_sample_count; }

  size_t MaxBatchSize() const { return batch_size; }

  // load input to device memory
  void LoadSampleToRam(mlperf::QuerySampleIndex sample_index,
    ...
    for (size_t input_index = 0; input_index < num_inputs; input_index++) {
      ...
      if (sample_memory_size[input_index] + input_size >
          (performance_sample_count + batch_size) *
              model->input_sizes[input_index])
        std::cerr
            << "warning: memory exceeded; try increasing model->input_sizes"
            << std::endl;

      // write to end of memory
      sample_memory_offset[input_index][index_in_memory] =
          sample_memory_size[input_index];
  ...
  void FinishLoading() {
    ...
    for (size_t k = 0; k < batch_size - 1; k++) {
      ...
      for (size_t i = 0; i < num_inputs; i++)
        device->Read(0, data[i], GetMemoryAddress(i, 0, index_in_memory),
                     samples[index_in_memory].size[i]);
      // LoadSampleToRam copies samples[index_in_memory] to end of memory
      LoadSampleToRam(samples[index_in_memory].index, data,
                      samples[index_in_memory].size,
                      samples[index_in_memory].shape);
    }
    ...
    for (size_t start = 0; start < num_samples_in_memory; start++) {
      ...
      for (size_t end = start;
           end < std::min(start + batch_size, num_samples_in_memory); end++) {
  ...
  void IssueBatch(size_t concurrency_index,
    ...
    //           << " running batch #" << batch.front().index << "-#" <<
    //           batch.back().index
    //           << " (" << (contiguous ? "contiguous" : "incontiguous") << ")"
    //           << std::endl;
    ...
    std::unique_lock<std::mutex> batch_memory_lock{
        batch_memory_mutex[memory_index], std::defer_lock};
    if (!contiguous)
    ...
    for (size_t i = 0; i < num_inputs; i++) {
      ...
      if (contiguous) {
        batch_data[i] =
            GetMemoryAddress(i, memory_index, node->index_in_memory);
      } else {
        // copy data if not contiguous
        for (size_t k = 0; k < batch.size(); k++) {
          const mlperf::QuerySample &sample = batch[k];
          void *sample_address = GetMemoryAddress(
              i, memory_index, sample_map[sample.index].index_in_memory);
          void *batch_sample_address =
              GetBatchMemoryAddress(i, memory_index, k);
          device->Copy(memory_index, batch_sample_address, sample_address,
                       GetSampleSize(sample.index, i));
        }
  ...
  void *GetMemoryAddress(size_t input_index, size_t memory_index,
                         size_t index_in_memory) const {
    size_t offset = sample_memory_offset[input_index][index_in_memory];
    return static_cast<uint8_t *>(sample_memory[input_index][memory_index]) +
           offset;
  }

  void *GetBatchMemoryAddress(size_t input_index, size_t memory_index,
                              size_t index_in_memory) const {
    size_t offset = index_in_memory * model->input_sizes[input_index];
    return static_cast<uint8_t *>(batch_memory[input_index][memory_index]) +
           offset;
  }

  const std::vector<size_t> &
  GetSampleShape(mlperf::QuerySampleIndex sample_index, size_t input_index) {
    return sample_map[sample_index].shape[input_index];
  }

  size_t GetSampleSize(mlperf::QuerySampleIndex sample_index,
                       size_t input_index) {
  ...
  /**
   * @brief Runs inference on a batch of samples and calls
   * mlperf::QuerySamplesComplete
   *
   * @param concurrency_index which device concurrency (device/core/thread) to
   * run on
   * @param batch the indices of the input
   * @param batch_data pointer to inputs in the device memory
   */
  virtual void RunInference(size_t concurrency_index,
                            const std::vector<mlperf::QuerySample> &batch,
...
private:
  // sample_memory[input_index][memory_index] points to loaded input buffer in
  // device memory
  ...
  // sample_memory_offset[input_index][index_in_memory] is the offset to a
  // sample input
  std::vector<std::vector<size_t>> sample_memory_offset;
  // batch_memory[input_index][memory_index] points to working memory for a
  // batch
...
public:
  DummyBackend(std::shared_ptr<Model> &model,
               std::shared_ptr<MlcDevice> &device,
               size_t performance_sample_count, size_t batch_size)
