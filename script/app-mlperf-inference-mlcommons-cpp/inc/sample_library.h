 ...
 * them to Backend. Derived classes specify how samples are read (e.g. from .npy
 * files)
 */
class SampleLibrary : public mlperf::QuerySampleLibrary {
public:
  SampleLibrary(const std::string &name, std::shared_ptr<MlcBackend> &backend,
                size_t max_sample_count, size_t num_inputs)
      : name(name), backend(backend), max_sample_count(max_sample_count),
  ...
  size_t TotalSampleCount() override {
    return max_sample_count != 0 ? std::min(max_sample_count, NumSamples())
                                 : NumSamples();
  }

  void LoadSamplesToRam(
      const std::vector<mlperf::QuerySampleIndex> &samples) override {
    std::cerr << "loading samples to ram with total sample size: "
  ...
  virtual void GetSample(mlperf::QuerySampleIndex sample_index,
                         size_t input_index, std::vector<uint8_t> &data,
                         size_t &size, std::vector<size_t> &shape) = 0;
...
public:
   ...
   */
  NumpyLibrary(std::shared_ptr<MlcBackend> &backend, size_t max_sample_count,
  ...
  size_t NumSamples() override { return file_paths.size(); }

  void GetSample(mlperf::QuerySampleIndex sample_index, size_t input_index,
...
public:
  Imagenet(std::shared_ptr<MlcBackend> &backend, size_t max_sample_count,
           std::string preprocessed_path, std::string val_map_path)
      : NumpyLibrary(backend, max_sample_count, preprocessed_path,
                     ReadValMap(val_map_path)) {
    std::cerr << "loaded imagenet with " << TotalSampleCount() << " samples"
              << std::endl;
  }
...
public:
  Openimages(std::shared_ptr<MlcBackend> &backend, size_t max_sample_count,
             std::string preprocessed_path, std::string annotations_path)
      : NumpyLibrary(backend, max_sample_count, preprocessed_path,
                     ReadAnnotations(annotations_path, max_sample_count)) {
    std::cerr << "loaded openimages with " << TotalSampleCount() << " samples"
              << std::endl;
  }

private:
  static const std::vector<std::string>
  ReadAnnotations(std::string annotations_path, size_t max_sample_count) {
    std::vector<std::string> filenames;
    ...
    while (std::regex_search(annotations, match, image_regex) &&
...
public:
  Squad(std::shared_ptr<MlcBackend> &backend, size_t max_sample_count,
    ...
    std::string input_ids_path =
        tokenized_root + "/bert_tokenized_squad_v1_1_input_ids.raw";
    std::string input_mask_path =
        tokenized_root + "/bert_tokenized_squad_v1_1_input_mask.raw";
    std::string segment_ids_path =
        tokenized_root + "/bert_tokenized_squad_v1_1_segment_ids.raw";
  ...
  size_t NumSamples() override { return num_samples; }
  ...
  void GetSample(mlperf::QuerySampleIndex sample_index, size_t input_index,
    ...
    if (input_index == 0)
      source = &input_ids_data;
    else if (input_index == 1)
      source = &input_mask_data;
    else
      source = &segment_ids_data;

    data.assign(source->begin() + offset,
                source->begin() + offset + sample_bytes);
    size = sample_bytes;
