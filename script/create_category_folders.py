...
def main():
    ...
    for category, scripts in sorted(categories.items()):
        ...
        with open(readme_path, 'w') as f:
            f.write(f"# {category}\n\n")
            f.write(f"This folder contains {len(scripts)} scripts in the **{category}** category.\n\n")
            f.write("## Scripts\n\n")
        ...
        for script_name in scripts:
            ...
            try:
                ...
                print(f"Warning: Could not create symlink for {script_name}: {e}")
    ...
    with open(main_readme, 'w') as f:
        ...
        f.write("This directory provides a category-based view of all scripts using symbolic links.\n\n")
        f.write(f"## Categories ({len(categories)})\n\n")
        for category in sorted(categories.keys()):
            folder_name = category_to_foldername(category)
            script_count = len(categories[category])
            f.write(f"- [{category}]({folder_name}/) ({script_count} scripts)\n")
