...
def preprocess(i):
    ...
    if path != '':
        if not os.path.isdir(path):
            return {'return': 1,
                    'error': 'path to dataset "{}" doesn\'t exist'.format(path)}
    ...
    if size == 'small' and tp == 'val' and ver == '2017':
        ...
        url_data_full = 'https://www.dropbox.com/scl/fi/whokyb7b7hyjqqotruyqb/{}?rlkey=hhgt4xtir91ej0nro6h69l22s&dl=0'.format(
            filename_data)
        url_ann_full = 'https://www.dropbox.com/scl/fi/bu41y62v9zqhee8w7q6z3/{}?rlkey=seqtgozldkc0ztu76kbd47p5w&dl=0'.format(
            filename_annotation)
    ...
    if path_from != '':
        ...
        if not os.path.isfile(path_from_data):
            return {'return': 1,
                    'error': 'File {} not found'.format(path_from_data)}
        ...
        if not os.path.isfile(path_from_annotation):
            return {'return': 1, 'error': 'File {} not found'.format(
