...
AOCL_VERSION="${MLC_AOCL_FFTZ_VERSION:-${MLC_GIT_CHECKOUT:-unknown}}"
if [[ -n "${MLC_OUTDIRNAME}" ]]; then
    INSTALL_PREFIX="${MLC_OUTDIRNAME}/aocl-fftz/${AOCL_VERSION}"
else
    INSTALL_PREFIX="${MLC_AOCL_FFTZ_SRC_PATH}/install"
fi
