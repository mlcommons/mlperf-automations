...
def postprocess(i):
    ...
    if is_true(env.get('MLC_AOCL_LIB_PATH_PROVIDED', '')):
        lib_path = env.get('MLC_AOCL_LIBM_LIB_PATH', '')
        if not lib_path or not os.path.isdir(lib_path):
            return {'return': 1, 'error': 'Provided MLC_AOCL_LIBM_LIB_PATH does not exist: ' + str(lib_path)}
        env['MLC_AOCL_LIBM_LIB_PATH'] = lib_path
        env['+LIBRARY_PATH'] = [lib_path]
        env['+LD_LIBRARY_PATH'] = [lib_path]
        env['MLC_DEPENDENT_CACHED_PATH'] = lib_path
        if not env.get('MLC_AOCL_LIBM_VERSION'):
            env['MLC_AOCL_LIBM_VERSION'] = _detect_version_from_path(lib_path) or 'unknown'
        return {'return': 0}
    ...
    if is_true(env.get('MLC_AOCL_BINARY_DOWNLOAD', '')):
        ...
        for entry in os.listdir(install_path):
            ...
            if os.path.isdir(entry_path) and (os.path.isdir(os.path.join(entry_path, 'lib')) or os.path.isdir(os.path.join(entry_path, 'lib64'))):
                ...
                break
        env['MLC_AOCL_LIBM_VERSION'] = env.get('MLC_AOCL_BINARY_VERSION', '')
    else:
        # Case 3: source build
        ...
        if env.get('MLC_OUTDIRNAME', ''):
            version = env.get('MLC_AOCL_LIBM_VERSION', env.get('MLC_GIT_CHECKOUT', 'unknown'))
            install_path = os.path.join(env['MLC_OUTDIRNAME'], 'aocl-libm', version)
        if not env.get('MLC_AOCL_LIBM_VERSION'):
            env['MLC_AOCL_LIBM_VERSION'] = env.get('MLC_GIT_REPO_CURRENT_HASH', env.get('MLC_GIT_CHECKOUT', 'unknown'))
    ...
    if is_true(env.get('MLC_AOCL_BINARY_DOWNLOAD', '')):
        _lp = os.path.join(install_path, 'lib')
        if not os.path.isdir(_lp):
            _lp = os.path.join(install_path, 'lib64')
        lib_path = _lp
    env['MLC_AOCL_LIBM_LIB_PATH'] = lib_path
    ...
    version = env.get('MLC_AOCL_LIBM_VERSION', '')
