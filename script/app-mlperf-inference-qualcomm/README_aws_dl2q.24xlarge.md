...
mlcr get,python --version_min=3.8.1
```
...
```
mlcr generate-run-cmds,inference,_performance-only --device=qaic \
...
### Full valid run
```
mlcr generate-run-cmds,inference,_submission --device=qaic \
...
```
mlcr get,dataset,imagenet,original,_full --env.IMAGENET_PATH=`pwd`/imagenet-2012-val
```

### Quick performance run

```
mlcr generate-run-cmds,inference,_performance-only --device=qaic --backend=glow \
...
```
mlcr generate-run-cmds,inference,_submission --device=qaic --backend=glow \
--scenario=Offline  --implementation=kilt --model=resnet50 \
